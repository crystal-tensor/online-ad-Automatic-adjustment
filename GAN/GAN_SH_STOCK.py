...
csv_file = 'sh_stock.csv'
f = open(csv_file, 'r', encoding=u'utf-8', errors='ignore')
df = pd.read_csv(f)
df.dropna(inplace=True)
x_data=preprocessing.minmax_scale(df.iloc[:, 3:4].values,feature_range=(-1,1))

# Hyper Parameters
BATCH_SIZE = 62
LR_G = 0.0001           # learning rate for generator
LR_D = 0.0001           # learning rate for discriminator
N_IDEAS = 1             # think of this as number of ideas for generating an art work (Generator)
ART_COMPONENTS = 120     # it could be total point G can draw in the canvas
...
def artist_works():     # painting from the famous artist (real target#
    a = np.random.uniform(0, 1, size=BATCH_SIZE)[:, np.newaxis]
    paintings =np.power(x_data,1)
    paintings = 1.68 * PAINT_POINTS[:] +np.sin(PAINT_POINTS[:]*10)
    #print(paintings)
    return paintings
