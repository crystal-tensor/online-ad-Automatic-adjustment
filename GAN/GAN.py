...
PAINT_POINTS = np.vstack([np.linspace(0.01, 4, ART_COMPONENTS) for _ in range(BATCH_SIZE)])
...
def artist_works():     # painting from the famous artist (real target)
    ...
    #paintings = 1.68 * PAINT_POINTS[:] +np.sin(PAINT_POINTS[:]*10) + a[:]
...
for step in range(5000):
    ...
    if step % 50 == 0:  # plotting
        ...
        plt.plot(PAINT_POINTS[0], 1 * PAINT_POINTS[0] + np.sin(PAINT_POINTS[0]*10) + 0, c='#FF9359', lw=3, label='lower bound')
